...
describe("Carousel", function() {
	...
	it("Should have the right state at the begin", function () {
		expect(componentInstance.state.selectedImage).toBe(0);
	...
	describe("Moving", function () {
		...
		it("Should set the selectedImage from the props", function () {
			componentInstance.setProps({selectedImage: 3});
			expect(componentInstance.state.selectedImage).toBe(3);
		});

		it("Should update the position of the carousel if selectedImage is changed", function () {
			componentInstance.setProps({selectedImage: 2});
			expect(componentInstance.state.selectedImage).toBe(2);
		...
		it("Should limit the position of the carousel firstItem to the `lastPosition`", function () {
			...
			componentInstance.setProps({selectedImage: 5});
			expect(componentInstance.state.selectedImage).toBe(5);
			expect(componentInstance.state.firstItem).toBe(3);

			componentInstance.setProps({selectedImage: 6});
			expect(componentInstance.state.selectedImage).toBe(6);
			expect(componentInstance.state.firstItem).toBe(3);
		});
	})

	describe("Selecting", function () {
		it("Should set the index as selectedImage when clicked", function () {
			expect(componentInstance.state.selectedImage).toBe(0);
			
			TestUtils.Simulate.click(findByTag(componentInstance, 'li')[1].getDOMNode());
			expect(componentInstance.state.selectedImage).toBe(1);

			TestUtils.Simulate.click(findByTag(componentInstance, 'li')[3].getDOMNode());
			expect(componentInstance.state.selectedImage).toBe(3);
		});

		it("Should call a given onSelectImage function when an item is clicked", function () {
			var mockedFunction = jest.genMockFunction();
			
			componentInstance.setProps({onSelectImage: mockedFunction});
	...
	describe("Navigating", function () {
		...
		it("Should disable the left arrow if we are showing the first item", function () {
			componentInstance.setProps({selectedImage: 0});
			expect(findByClass(componentInstance, 'control-arrow control-left control-disabled').length).toBe(1);
		});	

		it("Should enable the left arrow if we are showing other than the first item", function () {
			componentInstance.setProps({selectedImage: 1});
			expect(findByClass(componentInstance, 'control-arrow control-left control-disabled').length).toBe(0);
		});	
		...
		it("Should disable the right arrow if we reach the lastPosition", function () {
			...
			componentInstance.setProps({selectedImage: 4});
